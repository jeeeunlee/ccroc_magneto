...
inline std::ostream &operator<<(std::ostream &os, const GVect<T> &v) {
  ...
  for (unsigned int i = 0; i < v.size() - 1; i++)
    os << std::setw(stdprecision+4) << std::setprecision(stdprecision) << v[i] << ", ";
  os << std::setw(stdprecision+4) << std::setprecision(stdprecision) << v[v.size() - 1] << std::endl;
...
std::ostream &operator<<(std::ostream &os, const GMatr<T> &m) {
  ...
  for (unsigned int i = 0; i < m.nrows(); i++) {
    for (unsigned int j = 0; j < m.ncols() - 1; j++)
      os << std::setw(stdprecision+4) << std::setprecision(stdprecision) << m[i][j] << ", ";
    os << std::setw(stdprecision+4) << std::setprecision(stdprecision) << m[i][m.ncols() - 1]
       << std::endl;
  }
